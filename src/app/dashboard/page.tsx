...
function DashboardMainInner() {
  ...
  const [isDashDragOver, setIsDashDragOver] = useState(false);

  const handleDashDragOver = (e: React.DragEvent) => {
    e.preventDefault();
    setIsDashDragOver(true);
  };

  const handleDashDragLeave = () => {
    setIsDashDragOver(false);
  };
                ...
                <form onSubmit={submitDashProof} className="space-y-3 pt-2 border-t border-white/[0.06]">
                  ...
                  <div className="space-y-1.5 pt-1">
                    ...
                    <div
                      ...
                      onClick={() => document.getElementById("dash-proof-file-input")?.click()}
                      ...
                      } ${dashSubmitting ? "opacity-50 pointer-events-none" : ""}`}
                    >
                      <input
                        id="dash-proof-file-input"
                        type="file"
                        multiple
                      ...
                      <div className={`p-2.5 rounded-full transition-all duration-300 ${
                        isDashDragOver ? "bg-indigo-500/20 text-indigo-400" : "bg-white/[0.04] text-zinc-400 group-hover:text-zinc-300"
                      }`}>
                        <svg
                      ...
                      <div className="space-y-1">
                        <p className="text-xs font-semibold text-[#EAEFF8]">
                          {isDashDragOver ? "Drop to upload files" : "Drag & drop files here, or click to browse"}
                        </p>
                        ...
                        {dashProofFiles.map((f, i) => (
                          <li key={`${f.name}-${i}`} className="flex items-center justify-between gap-2 bg-white/[0.02] border border-white/[0.05] rounded-lg px-3 py-1.5 transition-all hover:bg-white/[0.04]">
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                setDashProofFiles(prev => prev.filter((_, idx) => idx !== i));
                              }}
